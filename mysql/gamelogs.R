...
setwd("~")
sql <- read_file("Github/baseball_etl/mysql/create_games_table.sql")

dbSendQuery(foo, strsplit(sql,";")[[1]][1])
dbSendQuery(foo, strsplit(sql,";")[[1]][2])

# TryCatch so it won't drop an old table on failure
...
for (k in 1871:2016) {
  ...
  dbWriteTable(foo, value = z, name = "games_temp", append = TRUE,row.names=F )
}
  dbSendQuery(foo,"DROP TABLE IF EXISTS retrosheet.games")
  dbSendQuery(foo,"ALTER TABLE retrosheet.games_temp rename to  games")}
  )