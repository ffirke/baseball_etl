...
for (i in 1930:2016){
  print(i)
  x = Sys.time()
  z = read_csv(paste0("C:/Users/Frank/Documents/Blog/Retrosheet/R_script/download.folder/unzipped/all",i, ".csv")
             ,col_names=cols,
             col_types = types) %>% as.data.frame()

  dbWriteTable(foo, value = z, name = "revised_events", append = TRUE,row.names=F )
  print(Sys.time()-x)
  print(Sys.time()-y)
}